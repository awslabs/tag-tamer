...
class SetTagGroup:
    ...
    def create_tag_group(
        self, tag_group_name, tag_group_key_name, tag_group_value_options
    ):
        if len(tag_group_name) and len(tag_group_key_name):
            try:
                ...
                log.debug(
                    'Successfully created Tag Group "%s" with Key "%s" & possible values "%s"',
                    tag_group_name,
                    tag_group_key_name,
                ...
                if (
                    ...
                    status_message = (
                        ...
                        + " - You are not authorized to create Tag Groups."
                    )
            ...
            self.my_status.error(
                message="Please provide a value for Tag Group name and Tag Key name"
            )
    ...
    def update_tag_group(
        ...
        try:
            ...
            log.debug(
                'Successfully updated Tag Group "%s" with Key "%s" to possible values "%s"',
                tag_group_name,
                tag_group_key_name,
                tag_group_value_options,
            )
            ...
            if (
                ...
                status_message = (
                    error.response["Error"]["Code"]
                    + " - You are not authorized to update Tag Groups."
                )
                self.my_status.error(message=status_message)
            else:
                self.my_status.error(message=error.response["Error"]["Message"])
