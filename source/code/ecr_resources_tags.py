...
# Getters & Setters for Amazon ECR repositories resource tags
# This class supports the main "resources_tags" class
# Included class & methods
# class - EcrResourcesTags
#  method - get_ecr_names_ids
#  method - get_ecr_resources_tags
#  method - get_ecr_tag_keys
#  method - get_ecr_tag_values
#  method - set_ecr_resources_tags
...
class EcrResourcesTags:
    ...
    def get_ecr_names_ids(self, filter_tags, **session_credentials):
        ...
        try:
            ...
            my_resources = client.describe_repositories()
            if "Repositories" in my_resources.keys():
                for repository in my_resources.get("Repositories"):
    ...
    # List of chosen resources from get_ecr_names_ids() & session credentials are arguments
    def get_ecr_resources_tags(self, chosen_resources, **session_credentials):
        my_status = ExecutionStatus()
            ...
            if chosen_resources:
                for resource_id_name in chosen_resources:
                    ...
                    try:
                        ...
                        # Extract the requested ECR repository from the returned list
                        repository = my_resources[0]
                        repository_arn = repository.get("repositoryArn")
                        tag_response = client.list_tags_of_resource(
                            ResourceArn=repository_arn
                        )
    ...
    def get_ecr_tag_keys(self, **session_credentials):
        my_status = ExecutionStatus()
        tag_keys_inventory = []
        ...
        if "Repositories" in my_resources.keys():
            for repository in my_resources.get("Repositories"):
                repository_arn = repository.get("repositoryArn")
                tag_response = client.list_tags_of_resource(ResourceArn=repository_arn)
                if "tags" in tag_response.keys():
            ...
            my_status.warning(message="No Amazon ECR repositories found!")
    ...
    # session credentials as the only input arguments
    def get_ecr_tag_values(self, **session_credentials):
        my_status = ExecutionStatus()
    ...
    def set_ecr_resources_tags(
        ...
        for resource_arn in self.resources_to_tag:
            try:
                ...
                my_status.success(message="ECR tags updated successfully!")
            except botocore.exceptions.ClientError as error:
                log.error("Boto3 API returned error: {}".format(error))
